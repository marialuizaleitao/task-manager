...
> Login, cadastro, lista de tarefas, compartilhamento e painéis de integração com o Google Calendar e o Telegram.
...
|---|---|
| `pages/` | Uma página por rota (Login, Register, Home, Categories, Tasks, SharedTasks), que orquestra dados e composição de componentes |
...
Dois índices compostos (`Task.Meta.indexes`), `(owner, due_date)` e `(owner, -created_at)`, foram adicionados na Sprint 5 depois de confirmar via `QuerySet.explain()` que filtrar ou ordenar por esses campos forçava um `TEMP B-TREE` mesmo após a redução por `owner_id` via índice. Nenhum outro índice foi criado: filtro por `completed` (baixa cardinalidade) e por `category` (já indexado pela FK) não mostraram esse padrão.
...
**Retries e timeout:** `GoogleCalendarClient` usa `httpx.HTTPTransport(retries=GOOGLE_API_MAX_RETRIES)`, que cobre falhas de conexão (DNS, timeout de conexão), mas não substitui o backoff exponencial documentado pelo Google para respostas `429`/`5xx` de quota, que exigiria uma fila para ser feito com segurança fora do ciclo request/response e fica registrado como débito técnico. O timeout (`GOOGLE_API_TIMEOUT_SECONDS`, padrão 10s) limita o pior caso de latência adicionada à criação/edição de uma tarefa.
...
- **`NotificationEvent`** (`interfaces.py`): um `dataclass` com `key` (string com namespace pontilhado, ex. `"task.created"`, `"task.shared"`, `"calendar.sync_failed"`), `user` (destinatário), `subject` (o objeto de domínio ao qual o evento se refere, hoje sempre uma `Task`) e `context` (dados que não vêm de `subject` sozinho, ex. quem fez uma alteração). Um evento com múltiplos destinatários (ex. "tarefa compartilhada foi atualizada", que avisa vários usuários afetados) é despachado como múltiplos `NotificationEvent`, um por destinatário, mantendo `notify()` de destinatário único, sem exigir que cada provedor implemente sua própria lógica de fan-out.
...
| `FilterSet` declarativo em `tasks`, `if` simples em `categories` | `FilterSet` em todo lugar | `tasks` tem 6 filtros combináveis; `categories` tem 1, e a complexidade do `FilterSet` só se paga a partir de um certo número de filtros |
...
O projeto adota [Versionamento Semântico](https://semver.org/lang/pt-BR/) (`MAJOR.MINOR.PATCH`). `CHANGELOG.md` segue o formato [Keep a Changelog](https://keepachangelog.com/pt-BR/1.1.0/), atualizado a cada conjunto de mudanças relevante. A versão atual é a `v1.0.0`; ver "Roadmap" e o `CHANGELOG.md` para o que ela contém.
...
O container `backend` aplica `migrate` e `collectstatic` automaticamente no start (`backend/docker-entrypoint.prod.sh`), sem necessidade de rodá-los manualmente a cada subida.
...
`.github/workflows/release.yml` roda via disparo manual (`workflow_dispatch`): valida o formato da versão, confirma que existe a seção correspondente no `CHANGELOG.md`, cria a tag e a Release no GitHub a partir dela.
...
Não configurada automaticamente pelo projeto (mudança de configuração do repositório, não de código), documentada aqui para referência. Em **Settings → Branches**, para `develop` e `master`:
...
**Rodando fora do Docker:** `POSTGRES_HOST` no `.env` é `db`, o nome do serviço na rede interna do Compose, que só resolve dentro de um container. Para rodar `pytest` nativo (Windows, Linux ou macOS, fora do `docker compose exec`), suba apenas o banco e aponte para `localhost` (a porta 5432 já é publicada em `docker-compose.yml`):
...
No PowerShell, defina a variável com `$env:POSTGRES_HOST="localhost"` antes do comando. Sem isso, a suíte inteira falha com `OperationalError: failed to resolve host 'db'`. Com `pytest-cov` instrumentado, esse tipo de falha em massa também aparece como uma cobertura artificialmente baixa (a maior parte do código nunca chega a ser exercitada), não é uma regressão real.
...
| 11 | Auditoria final, hardening, documentação definitiva e release `v1.0.0` | Concluído |

> A Sprint 7.1 foi um refinamento arquitetural sobre o escopo já entregue na Sprint 7 (generalização do canal de comunicação, novos eventos de compartilhamento e sincronização, resumo semanal), sem alterar a numeração nem o status das sprints acima.

> A Sprint 8 entregou toda a infraestrutura e documentação necessárias para publicar o projeto: Docker, Nginx, HTTPS, hardening e o guia completo em `docs/deploy-aws.md`, usado para provisionar a instância EC2 e publicar a aplicação.

> A Sprint 9 não adicionou nenhuma funcionalidade: o pipeline de CI (`.github/workflows/ci.yml`) roda em todo Pull Request para `develop`/`master`; a publicação de imagens (`publish.yml`) e o registro dos Environments (`deploy.yml`) rodam fora do PR; e `release.yml` executa manualmente, quando alguém decide cortar uma nova versão.

> A Sprint 11 também não adicionou nenhuma funcionalidade nova: foi uma sprint de auditoria, hardening e documentação final, sobre o escopo já entregue nas sprints anteriores.
